...
module.exports = (handlerName) => {
    ...
    const _stripQuery = (targetURL) => {
        let { protocol, host, pathname, query } = parse(targetURL, true)
        ...
        if (newQuery.length) {
            sanitizedTarget = `${protocol}//${host}${pathname}?${newQuery}`
        } else {
            sanitizedTarget = `${protocol}//${host}${pathname}`
        }
    ...
    const endWithError = (res, err = {}) => {
        ...
        console.log(`[ERROR] handler '${_handlerName}' (${_target}): ${message}\nerror trace:`, (errorObject || '(not given)'));
    }
